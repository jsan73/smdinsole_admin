        ...
        <div id="login-box" class="login_form">
          <div class="login_area">
            ...
            <ul v-else-if="step === 'FIND_PWD'">
              ...
              <li class="login_bar"><input type="text" class="input_txt" v-model="findInfo.email" placeholder="이메일 입력"></li>
              <li class="login_te">이름</li>
              <li class="login_bar"><input type="text" class="input_txt" v-model="findInfo.name" placeholder="이름 입력"></li>
  ...
  methods: {
      ...
      try {
        ...
        if (res.data.status === "SUCCESS") {
          ...
          if (this.pwdChange === "Y") {
            ...
            this.$router.replace({ name: 'PwdChange', params: { forceChange: 'Y', mgrId: this.loginId } });
          } else {
            this.completeLogin(res.data.data.token);
    ...
    async handleFindPwd() {
      try {
        ...
        if(res.data.status === "SUCCESS") {
          this.loginId = this.findInfo.email;
          this.rawEmail = res.data.data.mgrEmail;
    ...
    completeLogin(tokenData) {
      let payload = { adminId: this.loginId, autoLogin: "N", pwdChange: this.pwdChange };
